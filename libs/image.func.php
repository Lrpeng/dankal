 ...
 */
function waterText($filename,$text = "com.dankal",$fontfile = "MSYH.TTF"){
	$fileInfo = getimagesize($filename);
	$mime = $fileInfo['mime'];
	$createFun = str_replace("/", "createform", $mime);
	$outFun = str_replace("/",null, $mime);
 ...
 */
 function waterPic($dstFile,$srcFile = "../images/logo.jpg",$pct = 30){
	...
	$dsrMime = $dsrFileInfo ['mime'];
	$createSrcFun = src_replace("/","createform",$srcMime);
	$outDstFun = str_replace("/", null, $dstMime);
	$dst_im = $createDstFun($dstFile);
	$src_im = $createSrcFun($srcFile);
	imagecopymerge($dst_im, $dst_im , 0, 0, 0, 0, $src_w, $src_h, $pct);
